...
void handleEmbeddings(const httplib::Request& req, httplib::Response& res) {
    ...
    try {
        ...
        // 获取请求头
        struct curl_slist* headers = getHeaderManager().getApiHeaders(tempToken, HeaderManager::ApiType::EMBEDDINGS);

        // 将 JSON 重新序列化为紧凑格式（没有缩进和空格）
        std::string compressedJson = requestJson.dump(-1, ' ', false);

        // 创建连接，使用压缩后的 JSON
        CURL* curl = createEmbeddingsConnection(headers, compressedJson);
        if (!curl) {
